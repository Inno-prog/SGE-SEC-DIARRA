...
class EmployeeDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final empAsync = ref.watch(allEmployeesProvider).whenData(
          (list) => list.firstWhere((e) => e.id == id, orElse: () => throw Exception('Not found')),
        );
        ...
        body: NestedScrollView(
                ...
                background: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      EmployeeAvatar(photoUrl: employee.photoUrl, name: employee.fullName, radius: 40),
                      const SizedBox(height: 12),
                      Text(employee.fullName, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                      Text('${employee.poste} • ${employee.departementNom}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
          ...
          body: TabBarView(
            children: [
              ...
              _DocumentsTab(employeeId: employee.id, employeeNom: employee.fullName),
            ],
          ),
        ),
      ),
...
class _InfoTab extends ConsumerWidget {
  ...
  Future<void> _viewDocument(BuildContext context, WidgetRef ref, String? fileId, String fileName) async {
    if (fileId == null || fileId.isEmpty) {
      if (context.mounted) showSnack(context, 'Aucun document disponible', isError: true);
      return;
    }
    try {
    ...
    return ListView(
      ...
      children: [
        _InfoCard(title: 'Informations personnelles', items: [
          ...
          ('Sexe', employee.sexe),
          ('Date de naissance', DateFormat('dd/MM/yyyy').format(employee.dateNaissance)),
        ...
        _InfoCard(title: 'Informations professionnelles', items: [
          ...
          ('Salaire de base', '${NumberFormat('#,###').format(employee.salaire)} FCFA'),
          ('Ancienneté', '${employee.anciennete} an(s)'),
        ]),
        if (employee.contactUrgence != null) ...[
          const SizedBox(height: 16),
          _InfoCard(title: 'Contact d\'urgence', items: [
        ...
        const SizedBox(height: 16),
        _InfoCard(title: 'Solde de congés', items: [
        ...
        _DocCard(
          employee: employee,
          onViewDocument: (fileId, fileName) => _viewDocument(context, ref, fileId, fileName),
        ),
      ],
    );
  }
          ...
          children: [
            ...
            ...items.map((item) => Padding(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  SizedBox(width: 140, child: Text(item.$1, style: const TextStyle(color: AppColors.textSecondary, fontSize: 13))),
                  Expanded(child: Text(item.$2, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 13))),
                ],
              ),
            )),
          ],
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Documents', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: AppColors.primary)),
            const Divider(height: 20),
            ...docs.map((doc) {
              final label = doc.$1;
              final fileId = doc.$2;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return contracts.when(
      data: (list) => list.isEmpty
          ? const EmptyState(message: 'Aucun contrat', icon: Icons.description_outlined)
          : ListView.builder(
              padding: const EdgeInsets.all(16),
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(contract.type, style: const TextStyle(fontWeight: FontWeight.w600)),
        subtitle: Text(
        ...
        trailing: Column(
          ...
          children: [
            ...
            if (contract.isExpiringSoon)
              const Text('⚠️ Expire bientôt', style: TextStyle(fontSize: 10, color: AppColors.warning)),
          ],
        ),
      ),
    );
...
class _AttendanceTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final attendance = ref.watch(attendanceProvider(AttendanceParams(employeeId: employeeId)));
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      a.heureArrivee != null ? 'Arrivée: ${DateFormat('HH:mm').format(a.heureArrivee!)}${a.heureDepart != null ? ' • Départ: ${DateFormat('HH:mm').format(a.heureDepart!)}' : ''}' : '—',
                    ),
...
class _LeavesTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final leaves = ref.watch(leavesProvider(LeaveParams(employeeId: employeeId)));
    return leaves.when(
      data: (list) => list.isEmpty
          ? const EmptyState(message: 'Aucun congé', icon: Icons.beach_access_outlined)
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: list.length,
              itemBuilder: (_, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(l.type, style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: Text('${DateFormat('dd/MM/yyyy').format(l.dateDebut)} → ${DateFormat('dd/MM/yyyy').format(l.dateFin)} (${l.nombreJours}j)'),
...
class _PayrollTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final payroll = ref.watch(payrollProvider(PayrollParams(employeeId: employeeId)));
    final months = ['', 'Jan', 'Fév', 'Mar', 'Avr', 'Mai', 'Jun', 'Jul', 'Aoû', 'Sep', 'Oct', 'Nov', 'Déc'];
    return payroll.when(
      data: (list) => list.isEmpty
          ? const EmptyState(message: 'Aucune fiche de paie', icon: Icons.payments_outlined)
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: list.length,
              itemBuilder: (_, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text('${months[p.mois]} ${p.annee}', style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: Text('Brut: ${NumberFormat('#,###').format(p.brutTotal)} FCFA'),
                    trailing: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                        Text('${NumberFormat('#,###').format(p.netAPayer)} FCFA', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)),
          ...
          : ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  margin: const EdgeInsets.only(bottom: 8),
                  child: ListTile(
                    leading: const Icon(Icons.gavel_outlined, color: AppColors.error),
                    title: Text(s.type, style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: Text(s.motif),
                    trailing: Text(DateFormat('dd/MM/yyyy').format(s.date), style: const TextStyle(fontSize: 12)),
                  ),
                );
              },
            ),
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    trailing: Text(DateFormat('dd/MM/yyyy').format(t.date), style: const TextStyle(fontSize: 12)),
                  ),
                );
              },
            ),
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(d.nom, style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: Text(d.type),
                    trailing: Text(DateFormat('dd/MM/yyyy').format(d.createdAt), style: const TextStyle(fontSize: 12)),
                  ),
                );
              },
            ),
